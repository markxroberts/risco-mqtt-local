...
export function riscoMqttHomeAssistant(userConfig: RiscoMQTTConfig) {
  ...
  function publishInitialStates() {
    ...
    publishCloudStatus(panel.tcpSocket.socketMode, panel.proxy.cloudConnected)
  ...
  function panelOrMqttConnected() {
    ...
    if (!listenerInstalled) {
      ...
      panel.riscoComm.tcpSocket.on('CloudConnected', () => {publishCloudStatus(panel.tcpSocket.socketMode, true)});
      panel.riscoComm.tcpSocket.on('CloudDisconnected', () => {publishCloudStatus(panel.tcpSocket.socketMode, false)});
